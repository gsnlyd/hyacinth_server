defmodule HyacinthWeb.LabelSessionController do
  ...
  def label(conn, %{"label_session_id" => label_session_id, "element_index" => element_index}) do
    ...
    current_value = if length(labels) > 0, do: hd(labels).label_value, else: nil

    render(conn, "label.html", label_session: label_session, label_job: label_job, element: element, labels: labels, current_value: current_value,
           layout: {HyacinthWeb.LayoutView, "app_wide.html"})
  end
